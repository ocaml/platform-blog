...
for package to generate native executables.
...
This alpha requires a pre-existing Cygwin installation. Support for full
management of a local Cygwin environment inside of opam (so that it's as
transparent as possible to the user) is queued already and should be available
...
Recursive pinning allows to have opam lookup for opam files into
subdirectories, in different ways:

* `--recursive` option, opam goes through subdirectories to lookup for opam
...
`opam tree` show packages and their dependencies with a tree view. It is very
helpful to determine which packages brings what dependency in your installed
switch.
...
alias to `opam tree --rev-deps`), that is useful to examine how dependency
...
> Special thanks to @cannorin for collaborating on this feature

## Recommended development tools

There is now a way for a project maintainer to share their project developments
...
dependency, it will be ignored when installing normally, but pulled in when the
...
archived](https://www.softwareheritage.org/2021/04/20/connecting-ocaml/): the
...
To keep backwards compatibility of opam files, we added to the field
`url.mirrors:` (that is used to specify mirrors of the main url) a specific
Software Heritage URL syntax that is understood by opam 2.2.+ as a Software
...
> Special thank to @zapashcanon for collaborating on this feature
